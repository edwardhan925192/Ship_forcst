...
class Data_prep:
    def __init__(self, args):
        ...
        if args.load:
            self.train = self.load_data(self.filename1)
            self.test = self.load_data(self.filename2)
        if args.preprocess:
            self.train = self.preprocess(self.train)
            self.test = self.preprocess(self.test)
        if args.extract_dates:
            self.train = self.extract_date_parts(self.train, args.datetime_col)
            self.test = self.extract_date_parts(self.test, args.datetime_col)       
    ...
    def extract_date_parts(self, df, datetime_col):
